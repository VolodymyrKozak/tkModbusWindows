...
BOOL  RS232_vkCloseComport(int comport_number)
{
  BOOL res = CloseHandle(Cport[comport_number]);
  return res;
}
...
/* Ці дві функції добавив уже я, щоб мати можлививість розділити передачу модбас від прийому */
int RS232_OpenComportRead(int comport_number, int baudrate, const char *mode)
{
  if((comport_number>=RS232_PORTNR)||(comport_number<0))
  {
    printf("illegal comport number\n");
...
int RS232_OpenComportWrite(int comport_number, int baudrate, const char *mode)
{
  if((comport_number>=RS232_PORTNR)||(comport_number<0))
  {
    printf("illegal comport number\n");
